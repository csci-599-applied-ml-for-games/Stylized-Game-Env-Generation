...
def weights_init(m):
    ...
    elif classname.find('BatchNorm2d') != -1 and classname.find('Sync') == -1:
...
def define_G(input_nc, output_nc, prev_output_nc, ngf, which_model_netG, n_downsampling, norm, scale, loadSize, gpu_ids=[], opt=[]):
    netG = None    
    ...
    if which_model_netG == 'global':        
        ...
        netG = CompositeGenerator(opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, opt.n_blocks, opt.fg, opt.no_flow, norm_layer, loadSize=loadSize)
    elif which_model_netG == 'compositeLocal':
        netG = CompositeLocalGenerator(opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, loadSize, opt.n_blocks_local, opt.fg, opt.no_flow, norm_layer, scale=scale)

        # (netG_input_nc, opt.output_nc, prev_output_nc, ngf, opt.netG + 'Local', loadSize, opt.n_downsample_G, opt.norm, s, self.gpu_ids, opt))
...
class CompositeGenerator(BaseNetwork):
    def __init__(self, opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, n_blocks, use_fg_model=False, no_flow=False, norm_layer=nn.BatchNorm2d, padding_type='reflect', loadSize = 256):
        assert(n_blocks >= 0)
        ...
        ## TOP LEFT
        ## 1) Encoder
        ## 2) SpadeResnet blocks
        ##
        ## FG MODEL:
        ## Remove indv_down
        ## spade resnet instead of resnet in indv_res
        ## Linear to indv_lin
        ## enc->indv_lin->indv_res
        ## conv for seg masks
        ## change indv_final according to our arch

        if use_fg_model:
            ### individial image generation
            # ngf_indv = ngf // 2 if n_downsampling > 2 else ngf
            # indv_nc = input_nc
            # Pass ngf as 512 for coarsest, and 128 for finest
            self.sw, self.sh = self.loadSize, self.loadSize * 2
            indv_lin = [nn.Linear(opt.z_dim, ngf // (2 ** 4) * self.sw * self.sh)]

            indv_srb_seg = []
            for i in range(n_blocks - 4):
                indv_srb_seg += [SPADEResnetBlock(ngf // (2 ** 4), ngf // (2 ** 4), opt)]

            for i in range(4):
                indv_srb_seg += [SPADEResnetBlock(ngf // (2 ** (4 - i)), ngf // (2 ** (4 - i - 1)), opt)]

            # #downsample seg maps
            # indv_seg = []
            # # interpolate layer to be put in forward
            # for i in range(n_downsampling):
            #     indv_seg += [nn.Conv2d(input_nc, self.ngf, 3, padding=1)]

            indv_up = []
            for i in range(n_blocks - 3):
                indv_up += [SPADEResnetBlock(self.ngf, self.ngf, opt)]
            indv_up += [nn.Upsample(scale_factor=2)]
            for i in range(1, n_blocks - 3):
                indv_up += [SPADEResnetBlock(self.ngf // 2 ** (i - 1), self.ngf // 2 ** i, opt)]
            indv_up += [nn.Upsample(scale_factor=2)]

            indv_final = [nn.ReflectionPad2d(3), nn.Conv2d(self.ngf // 2 ** 3, output_nc, kernel_size=7, padding=0), nn.Tanh()]

        ## TOP LEFT NEW
        #Pass ngf as 512 for coarsest, and 128 for finest
        self.sw, self.sh = self.loadSize, self.loadSize * 2
        model_lin = [nn.Linear(opt.z_dim, ngf // (2 ** 4) * self.sw * self.sh)]

        model_srb_seg = []
        for i in range(n_blocks - 4):
            model_srb_seg += [SPADEResnetBlock(ngf // (2 ** 4), ngf // (2 ** 4), opt)]

        for i in range(4):
            model_srb_seg += [SPADEResnetBlock(ngf // (2 ** (4 - i)), ngf // (2 ** (4 - i - 1)), opt)]

        # # downsampling segmentation map
        # model_seg = []
        # # interpolate layer to be put in forward
        # for i in range(n_downsampling):
        ...
        model_up_flow = []
        for i in range(n_blocks):
            model_up_flow += [ResnetBlock(self.ngf, padding_type=padding_type, activation=activation, norm_layer=norm_layer)]

        if not no_flow:
            ### upsample
            for i in range(1, n_downsampling + 1):
                model_up_flow += [nn.ConvTranspose2d(self.ngf // 2 ** (i - 1), self.ngf // 2 ** i, kernel_size=3, stride=2, padding=1, output_padding=1), norm_layer(self.ngf // 2), activation]
            model_final_flow = [nn.ReflectionPad2d(3), nn.Conv2d(self.ngf, 2, kernel_size=7, padding=0)]
            model_final_w = [nn.ReflectionPad2d(3), nn.Conv2d(self.ngf, 1, kernel_size=7, padding=0), nn.Sigmoid()]

        if use_fg_model:
            self.indv_lin = nn.Sequential(*indv_lin)
            # self.indv_seg = nn.Sequential(*indv_seg)
            self.indv_srb_seg = nn.Sequential(*indv_srb_seg)
            self.indv_up = nn.Sequential(*indv_up)
            self.indv_final = nn.Sequential(*indv_final)

        #TOP LEFT LINEAR
        self.model_lin = nn.Sequential(*model_lin)
        #TOP LEFT RES
        self.model_srb_seg = nn.Sequential(*model_srb_seg)
        #SEG DOWN
    ...
    def forward(self, input, img_prev, mask, img_feat_coarse, flow_feat_coarse, img_fg_feat_coarse, use_raw_only):
        ...
        if not self.no_flow:
            flow_feat = self.model_up_flow(downsample)
...
class CompositeLocalGenerator(BaseNetwork):
    def __init__(self, opt, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, loadSize, n_blocks_local, use_fg_model=False, no_flow=False, norm_layer=nn.BatchNorm2d, padding_type='reflect', scale=1):
        ...
        #### Use loadsize for  sizing z here
