...
class Vid2VidModelG(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG0 = networks.define_G(netG_input_nc, opt.output_nc, prev_output_nc, opt.ngf, opt.netG, 
                                       opt.n_downsample_G, opt.norm, 0, opt.loadSize//(2**self.n_scales), self.gpu_ids, opt)
        for s in range(1, self.n_scales):            
            ngf = opt.ngf // (2**s)
            loadSize = opt.loadSize//(2**(self.n_scales - s))
            setattr(self, 'netG'+str(s), networks.define_G(netG_input_nc, opt.output_nc, prev_output_nc, ngf, opt.netG+'Local', loadSize, opt.n_downsample_G, opt.norm, s, self.gpu_ids, opt))
    ...
    def forward(self, input_A, input_B, inst_A, fake_B_prev, dummy_bs=0):
        ...
        fake_B, fake_B_raw, flow, weight = self.generate_frame_train(netG, real_A_all, real_B_all, fake_B_prev, start_gpu, is_first_frame)
    ...
    def generate_frame_train(self, netG, real_A_all, real_B_all, fake_B_pyr, start_gpu, is_first_frame):
        tG = self.opt.n_frames_G
